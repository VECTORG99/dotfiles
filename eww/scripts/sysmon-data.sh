...
jq -n \
  --arg cpu "$cpu" --arg cpu_temp "$cpu_temp" \
  --arg gpu "$gpu_usage" --arg gpu_temp "$gpu_temp" \
  --arg ram "$ram_info" --arg disk "$disk_info" \
  '{cpu:$cpu, cpu_temp:$cpu_temp, gpu:$gpu, gpu_temp:$gpu_temp, ram:$ram, disk:$disk}'
